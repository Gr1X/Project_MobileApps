...
class QueueViewModel(
    ...
    val uiState: StateFlow<QueueUiState> = combine(
        ...
        val myQueue = queues.find { it.userId == currentUser?.uid && it.status == com.example.project_mobileapps.data.model.QueueStatus.MENUNGGU }
        ...
        val queuesAhead = if (myQueue != null && status != null) {
            queues.count {
                it.doctorId == myQueue.doctorId &&
                        ...
                        it.status == com.example.project_mobileapps.data.model.QueueStatus.MENUNGGU
            }
        } else 0
